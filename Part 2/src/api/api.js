...
//Patch the percentage of a Task for a user with a new percentage value
export async function patchTask(userID, listid, taskid, taskname, done, deadline) {
    let patchobj = {};
    if(taskname !== '') {
        patchobj.name = taskname;
    } 
    if(done !== null) {
        patchobj.done = done;
    } 
    if(deadline !== '' && deadline !== undefined) {
        patchobj.deadline = deadline;
    } 
    const res = await fetch(baseUrl + 'users/' + userID + '/lists/' + listid + '/tasks/' + taskid, {
        method: 'PATCH',
        headers: {
